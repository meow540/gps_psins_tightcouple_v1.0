...
% Ensure all outputs are assigned even on early exit.
navSolutions = [];
ephTemplate = initEph();
...
for channelNr = activeChnList
    startIdx = subFrameStart(channelNr) - 20;
    endIdx   = subFrameStart(channelNr) + (1500 * 20) - 1;
    if (startIdx < 1) || (endIdx > length(trackResults(channelNr).I_P))
    ...
    ephTmp = ephTemplate;
    fields = fieldnames(ephDecoded);
    for fi = 1:numel(fields)
        ephTmp.(fields{fi}) = ephDecoded.(fields{fi});
    end
    eph(trackResults(channelNr).PRN) = ephTmp;
...
maxSubFrameStart = max(subFrameStart(activeChnList));
positioningTimes = fix((settings.msToProcess - maxSubFrameStart) / settings.navSolPeriod);  % Times of positioning
if positioningTimes < 1
    disp('Not enough data after first subframe for navigation solution. Exiting!');
    navSolutions = [];
    eph          = struct([]);
    return
end
