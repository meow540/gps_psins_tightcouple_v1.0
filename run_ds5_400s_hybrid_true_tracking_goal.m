...
outPrefix = 'rt_deep_goal2_ds5_hybrid_true_tracking_goal_400s';
resultFile = [outPrefix '_ds5_400s.mat'];
...
opts.datasets = {'ds5'};
opts.roundTime = 800;          % 400 s at 0.5 s nav period
opts.calibEndEpoch = 180;
opts.deepPerfDiagInterval = 20;

% Hybrid runtime: use precomputed main tracking, but keep true-reference
% shadow tracking on actual raw IF samples.
opts.deepFastMode = 1;
...
fprintf('\n=== DS5 400s hybrid true-tracking goal run ===\n');
...
if exist(resultFile, 'file') ~= 2
    ...
    fprintf('Expected exact 400s file was not created; using actual result file: %s\n', resultFile);
end
